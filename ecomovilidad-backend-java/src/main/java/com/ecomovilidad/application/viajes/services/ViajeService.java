...
public class ViajeService {
    ...
    private ViajeResponse toResponse(Viaje v) {
        String nombreRuta = rutaRepository.findById(v.getRutaId())
                .map(r -> r.getNombre()).orElse(null);
        String placaVehiculo = vehiculoRepository.findById(v.getVehiculoId())
                .map(vh -> vh.getPlaca()).orElse(null);
        String nombreConductor = conductorRepository.findById(v.getConductorId())
        ...
        return new ViajeResponse(
                ...
                nombreRuta, placaVehiculo, nombreConductor);
    }
