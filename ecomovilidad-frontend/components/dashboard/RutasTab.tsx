...
import { useRutas, type RutaDto, type CoordenadaDto, type PuntoIntermedioCoordenada } from '@/hooks/useRutas';
...
// Samplea los coords OSRM para guardarlos como puntosIntermedios (máx 50 pts)
function sampleOsrmCoords(coords: LL[], max = 50): PuntoIntermedioCoordenada[] {
  if (coords.length <= 2) return [];
  const inner = coords.slice(1, -1);
  const step = inner.length <= max ? 1 : Math.ceil(inner.length / max);
  return inner
    .filter((_, i) => i % step === 0)
    .map((c, i) => ({ latitud: c[0], longitud: c[1], orden: i }));
}
...
export default function RutasTab() {
  const { rutas, loading, crear, eliminar, activar, suspender } = useRutas();
  ...
  const handleCrear = async () => {
    ...
    const r = await crear({
      ...
      color: draft.color,
      puntosIntermedios: sampleOsrmCoords(draft.osrmCoords),
    });
    if (r) {
  ...
  const handleActivar = async (e: React.MouseEvent, id: string) => {
    e.stopPropagation();
    setActioningId(id);
    const ok = await activar(id);
    ok ? notificarExito('Ruta activada') : notificarError('Error al activar ruta');
    setActioningId(null);
  };

  const handleSuspender = async (e: React.MouseEvent, id: string) => {
    e.stopPropagation();
    setActioningId(id);
    const ok = await suspender(id);
    ok ? notificarExito('Ruta suspendida') : notificarError('Error al suspender ruta');
    setActioningId(null);
  };
                  ...
                  >
                    ...
                    <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-all duration-150">
                      {actioningId === ruta.id ? (
                        ...
                        <>
                          {(ruta.estado === 'Borrador' || ruta.estado === 'Suspendida') && (
                            <button
                              onClick={(e) => handleActivar(e, ruta.id)}
                              className="p-1 rounded-md transition-all duration-150 hover:text-emerald-400"
                          ...
                          {ruta.estado === 'Activa' && (
                            <button
                              onClick={(e) => handleSuspender(e, ruta.id)}
                              className="p-1 rounded-md transition-all duration-150 hover:text-yellow-400"
                              style={{ color: 'var(--text-muted)' }}
                          ...
                          <button
                            ...
                            className="p-1 rounded-md transition-all duration-150 hover:text-red-400"
                            style={{ color: 'var(--text-muted)' }}
                            title="Eliminar"
                          >
                            <Trash2 size={13} />
        ...
        {ready ? (
          <MapContainer key={`rutas-${theme}`} center={CENTER} zoom={12} className="h-full w-full" scrollWheelZoom zoomControl>
            ...
            {rutas.map((r) => {
              ...
              const routeColor = isSelected ? '#10B981' : (r.color || '#3b82f6');
              return (
                <div key={r.id}>
                  <Polyline
                  ...
                  />
                  {/* Origin dot — filled, larger */}
                  <CircleMarker
                    center={toLL(r.origen)}
                  ...
                  >
                    <Popup>
                      ...
                      <span style={{ fontSize: 11, color: '#6B7280' }}>
                        {r.distanciaTotalKm} km · {r.estado}
                      </span>
                    </Popup>
                  </CircleMarker>
                  {/* Destination dot — smaller */}
                  <CircleMarker
                    center={toLL(r.destino)}
